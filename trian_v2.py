...
parser.add_argument('--model', default='Hp60', help='Hp60, kp')
parser.add_argument('--lr', default=0.001, help='learning rate')
parser.add_argument('--epochs', default=10000, help='epoch number')
...
if model_selection == 'kp': doc = doc.drop(columns= 'Hp60')
doc.head()
...
input_length = 24
output_length = 1 
y_index = 2 if model_selection == 'Hp60' else 1
for i in range(len(normalized_data) - input_length - output_length + 1):
    x.append(normalized_data[i:i+input_length])  # Input sequence (24 time steps)
    y.append(normalized_data[i+input_length : i+input_length+output_length, y_index]) 
x = np.array(x)
y = np.array(y)
...
if __name__ == '__main__':
    print(f'\n------------- Training {model_selection} with lr: {learning_rate}, batch: {batch_size} -------------')
    
    ## Initialize the model, loss function, and optimizer
    # > Number of CNN hidden units -> 32
    # > Number of LSTM hidden units -> 2
    if model_selection == 'Hp60':
        model = CNNLSTMModel(32, 2, n_input= 3, n_output= 1).to(device)
    elif model_selection == 'kp':
        model = CNNLSTMModel(32, 2, n_input= 2, n_output= 1).to(device)
